...
class OracleCollaborativeAUC(tf.keras.metrics.AUC):
  ...
  def __init__(self,
    ...
    Args:
      ...
      multi_label: Whether multilabel data should be treated as multilabel, or
        flattened into a single binary label before AUC computation.
    """
    # Currently only binary data is supported.
    if multi_label:
    ...
    super(OracleCollaborativeAUC, self).__init__(
  ...
  def update_state(self,
    ...
    batch_binned_confusion_matrix = self._bin_confusion_matrix_by_score(
        pred_labels, true_labels, custom_binning_score, self.num_bins,
        self.dtype)
  ...
  @staticmethod
  def _bin_confusion_matrix_by_score(pred_labels: Sequence[Sequence[bool]],
                                     true_labels: Sequence[Sequence[bool]],
                                     binning_score: Sequence[Sequence[float]],
                                     nbins: int,
                                     dtype: tf.DType) -> Dict[str, tf.Tensor]:
    ...
    Args:
      ...
      nbins: Number of output bins.
      dtype: DType of output confusion matrix.
    ...
    # Cast confusion matrix elements from bool to dtype.
    pred_true_positives = tf.cast(pred_true_positives, dtype)
    pred_true_negatives = tf.cast(pred_true_negatives, dtype)
    pred_false_positives = tf.cast(pred_false_positives, dtype)
    pred_false_negatives = tf.cast(pred_false_negatives, dtype)

    bin_indices = tf.histogram_fixed_width_bins(
        binning_score, tf.constant([0., 1.], dtype), nbins=nbins)
    ...
    for tp_row, tn_row, fp_row, fn_row, idx_row in (zip(pred_true_positives,
                                                        pred_true_negatives,
      ...
      binned_true_positives_rows.append(
          tf.math.unsorted_segment_sum(
              data=tp_row, segment_ids=idx_row, num_segments=nbins))
      binned_true_negatives_rows.append(
          tf.math.unsorted_segment_sum(
              data=tn_row, segment_ids=idx_row, num_segments=nbins))
      binned_false_positives_rows.append(
          tf.math.unsorted_segment_sum(
              data=fp_row, segment_ids=idx_row, num_segments=nbins))
      binned_false_negatives_rows.append(
          tf.math.unsorted_segment_sum(
              data=fn_row, segment_ids=idx_row, num_segments=nbins))
  ...
  def _replace_first_and_last_elements(original_tensor: Sequence[float],
                                       new_first_elmt: float,
                                       new_last_elmt: float):
    """Replace the first and last elements of original_tensor."""
    return tf.concat([[new_first_elmt], original_tensor[1:-1], [new_last_elmt]],
                     axis=0)

  def result(self):
    """Computes the approximate Oracle-Collaborative AUC.
    ...
    Returns:
      The approximate Oracle-Collaborative AUC.
    """
    ...
    expected_true_positives += tf.reduce_sum(
        tf.where(cum_examples <= num_oracle_examples,
                 self.binned_true_positives + self.binned_false_negatives, 0.),
        axis=1)
    expected_true_negatives += tf.reduce_sum(
        tf.where(cum_examples <= num_oracle_examples,
                 self.binned_true_negatives + self.binned_false_positives, 0.),
        axis=1)
    ...
    last_complete_bin_indices = tf.stack([
    ...
    expected_false_positives += tf.reduce_sum(
        tf.where(cum_examples > num_oracle_examples,
                 self.binned_false_positives, 0.),
        axis=1)
    expected_false_negatives += tf.reduce_sum(
        tf.where(cum_examples > num_oracle_examples,
                 self.binned_false_negatives, 0.),
        axis=1)
    ...
    return super(OracleCollaborativeAUC, self).result()
